locals {
  ...
  # Merge attributes
  attributes = compact(concat(var.attributes, local.context_local["attributes"]))
  ...
  label_order_context_list = compact(local.context_local["label_order"])
  label_order_final_list = compact(
    coalescelist(
      var.label_order,
